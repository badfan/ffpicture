    ...
    protected void onResult(List<LocalMedia> images) {
        ...
        if (isAndroidQ && config.isAndroidQTransform) {
            ...
            for (int i = 0; i < images.size(); i++) {
                LocalMedia media = images.get(i);
                if (MimeType.eqVideo(media.getMimeType())) {
                    Bitmap videoThumbnail = ThumbnailUtils.createVideoThumbnail(
                            media.getPath(), MediaStore.Video.Thumbnails.MINI_KIND);
                    String thumPath = ImageUtils.saveImageToSD(this, System.currentTimeMillis() + "", videoThumbnail);
//                media.setThumbVideo(BitmapUtils.compress(videoThumbnail, 128 * 1024));
                    media.setThumbVideo(videoThumbnail);
                    media.setThumbVideoPath(thumPath);
                    Log.e("thumb", "封面原始大小：" + videoThumbnail.getByteCount() + ",压缩后大小：" + media.getThumbVideo().getByteCount());
                }
            }
...
//        AsyncTask.SERIAL_EXECUTOR.execute
        new Thread(() -> {
            ...
            Log.e("androidQ", "onResultToAndroidAsy1111111111");
            int size = images.size();
            Log.e("androidQ", "onResultToAndroidAsy22222");
            for (int i = 0; i < size; i++) {
                LocalMedia media = images.get(i);
            ...
            Log.e("androidQ", "onResultToAndroidAsy44444444");
            // 线程切换
