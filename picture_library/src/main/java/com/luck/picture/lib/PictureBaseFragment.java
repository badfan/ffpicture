 ...
 * @描述: Activity基类
 */
public abstract class PictureBaseFragment extends Fragment implements Handler.Callback {
...
//        ImmersiveManage.immersiveAboveAPI23(getActivity()
//                , colorPrimaryDark
//                , colorPrimary
//                , openWhiteStatusBar);
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (!config.camera) {
            getActivity().setTheme(config.themeStyleId);
        }
        ...
        if (config.style != null && config.style.pictureNavBarColor != 0) {
            NavBarUtils.setNavBarColor(getActivity(), config.style.pictureNavBarColor);
        }
    ...
    private void initConfig() {
        ...
        if (config.style != null) {
            ...
            if (!openWhiteStatusBar) {
                // 兼容老的Theme方式
                openWhiteStatusBar = AttrsUtils.getTypeValueBoolean(getActivity(), R.attr.picture_statusFontColor);
            }
            ...
            if (!numComplete) {
                // 兼容老的Theme方式
                numComplete = AttrsUtils.getTypeValueBoolean(getActivity(), R.attr.picture_style_numComplete);
            }
            ...
            if (!config.checkNumMode) {
                // 兼容老的Theme方式
                config.checkNumMode = AttrsUtils.getTypeValueBoolean(getActivity(), R.attr.picture_style_checkNumMode);
            }
            ...
            if (config.titleBarBackgroundColor != 0) {
                ...
                // 兼容老的Theme方式
                colorPrimary = AttrsUtils.getTypeValueColor(getActivity(), R.attr.colorPrimary);
            }
            ...
            if (config.pictureStatusBarColor != 0) {
                ...
                // 兼容老的Theme方式
                colorPrimaryDark = AttrsUtils.getTypeValueColor(getActivity(), R.attr.colorPrimaryDark);
            }
        }
    ...
    public void onAttach(@NonNull Context context) {
        ...
        if (config != null) {
            super.onAttach((PictureContextWrapper.wrap(context, config.language)));
        }
    }
    ...
    protected void startCrop(String originalPath, String mimeType) {
        if (TextUtils.isEmpty(originalPath)) {
            ToastUtils.s(getActivity(), getString(R.string.picture_not_crop_data));
            return;
        }
        ...
        if (config.cropStyle != null) {
            ...
            if (config.cropTitleBarBackgroundColor != 0) {
                ...
                // 兼容老的Theme方式
                toolbarColor = AttrsUtils.getTypeValueColor(getActivity(), R.attr.picture_crop_toolbar_bg);
            }
            if (config.cropStatusBarColorPrimaryDark != 0) {
                statusColor = config.cropStatusBarColorPrimaryDark;
            } else {
                // 兼容老的Theme方式
                statusColor = AttrsUtils.getTypeValueColor(getActivity(), R.attr.picture_crop_status_color);
            }
            if (config.cropTitleColor != 0) {
                titleColor = config.cropTitleColor;
            } else {
                // 兼容老的Theme方式
                titleColor = AttrsUtils.getTypeValueColor(getActivity(), R.attr.picture_crop_title_color);
            }
            ...
            if (!isChangeStatusBarFontColor) {
                // 是否改变裁剪页状态栏字体颜色 黑白切换
                isChangeStatusBarFontColor = AttrsUtils.getTypeValueBoolean(getActivity(), R.attr.picture_statusFontColor);
            }
        }
        ...
        UCrop.of(uri, Uri.fromFile(file))
                .withOptions(options)
                .startAnimationActivity(getActivity(), config.windowAnimationStyle != null
    ...
    protected void startCrop(ArrayList<CutInfo> list) {
        ...
        UCrop.of(uri, Uri.fromFile(file))
                .withOptions(options)
                .startAnimationMultipleCropActivity(getActivity(), config.windowAnimationStyle != null
                        ? config.windowAnimationStyle.activityCropEnterAnimation : R.anim.picture_anim_enter);
    }
    ...
    protected void closeActivity() {
        ...
        if (config.camera) {
            ...
            getActivity().overridePendingTransition(0, config.windowAnimationStyle != null
    ...
    protected int getLastImageId(String mimeType) {
        try {
            //selection: 指定查询条件
            String absolutePath = PictureFileUtils.getDCIMCameraPath(getActivity(), mimeType);
            String ORDER_BY = MediaStore.Files.FileColumns._ID + " DESC";
            String selection = MediaStore.Images.Media.DATA + " like ?";
            //定义selectionArgs：
            String[] selectionArgs = {absolutePath + "%"};
            Cursor data = getActivity().getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI, null,
    ...
    protected void startOpenCamera() {
        ...
        if (cameraIntent.resolveActivity(getActivity().getPackageManager()) != null) {
            ...
            if (SdkVersionUtils.checkedAndroid_Q()) {
                imageUri = MediaUtils.createImageUri(getActivity().getApplicationContext());
                if (imageUri != null) {
                ...
                File cameraFile = PictureFileUtils.createCameraFile(getActivity().getApplicationContext(),
    ...
    protected void startOpenCameraVideo() {
        ...
        if (cameraIntent.resolveActivity(getActivity().getPackageManager()) != null) {
            Uri imageUri;
            if (SdkVersionUtils.checkedAndroid_Q()) {
                imageUri = MediaUtils.createVideoUri(getActivity().getApplicationContext());
                if (imageUri != null) {
                    config.cameraPath = imageUri.toString();
                } else {
                    ToastUtils.s(getContext(), "open is camera error，the uri is empty ");
